...
public class DataManager {
    ...
    private static boolean isDinamicClustering = false;
    private static Cluster[] Clusters = null;
        ...
        if (e instanceof ScannedRobotEvent) {
            ...
            if (i.isTeammate(SRE.getName())) {
                ...
                if (num != RobotNotFound) {
                    TeammateData[num].updateItem(Utils.getX(i.getX(), SRE.getDistance(), SRE.getBearing()
                            + i.getRobotFrontHeading()), Utils.getY(i.getY(), SRE.getDistance(), SRE.getBearing()
                ...
                if (num != RobotNotFound) {
                    ...
                    if (isDinamicClustering) {
                        EnemyData[EnemyCount].addDinamicClustering(MyRobot, Clusters);
                    }
                    if (isGuessFactor) {
    ...
    public void sendMe() {
        if (MyRobot instanceof TeamRobot) {
            Me me = new Me((TeamRobot) MyRobot);
            try {
                ((TeamRobot) MyRobot).broadcastMessage(me);
            } catch (IOException ex) {
            }
        }
    }
    ...
    public void AddDinamicClustering(Cluster[] Clusters) {
        isDinamicClustering = true;
        this.Clusters = Clusters;
        for (int b = 0; b < EnemyCount; b++)
            EnemyData[b].addDinamicClustering(MyRobot, Clusters);
    }
