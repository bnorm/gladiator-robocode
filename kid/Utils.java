...
public class Utils {
   ...
   /**
    * Returns the absolute smaller of two <code>double</code> values. That is, the
    * result is the argument closer to zero. If the arguments have the same value, the
    * result is the first argument. If either value is <code>NaN</code>, then the
   ...
   /**
    * Returns the absolute greater of two <code>double</code> values. That is, the
    * result is the argument farther from zero. If the arguments have the same value, the
    * result is the first argument. If either value is <code>NaN</code>, then the
    * result is <code>NaN</code>.
    * 
    * @param a - a number.
   ...
   /**
    * Returns the absolute angle in degrees of the <code>double</code> value. That is,
    * the result is between <code>0.0</code> and <code>360.0</code>.
    * 
   ...
   /**
    * Converts rectangular coordinates (x, y) to compass (r, <i>theta</i>). This method
    * computes the phase <i>theta</i> by computing an arc tangent of x/y in the range of
    * <code>-180.0</code> to <code>180.0</code>. Special cases:
    * <ul>
    ...
    * the first argument is positive infinity and the second argument is positive and
    * finite, then the result is positive zero.
    * <li>If the first argument is positive and the second argument is negative zero, or
    * the first argument is positive infinity and the second argument is negative and
    * finite, then the result is negative zero.
    * <li>If the first argument is negative and the second argument is positive zero, or
    * the first argument is negative infinity and the second argument is positive and
    * finite, then the result is the double value closest to <code>180.0</code>.
    * <li>If the first argument is negative and the second argument is negative zero, or
    * the first argument is negative infinity and the second argument is negative and
    * finite, then the result is the double value closest to <code>-180.0</code>.
    * <li>If the first argument is positive zero or negative zero and the second argument
    * is positive, or the first argument is finite and the second argument is positive
    * infinity, then the result is the double value closest to <code>90.0</code>.
    * <li>If the first argument is positive zero or negative zero and the second argument
    * is negative, or the first argument is finite and the second argument is negative
    * infinity, then the result is the double value closest to <code>-90.0</code>.
    * <li>If both arguments are positive infinity, then the result is the double value
    * closest to <code>45.0</code>.
    * <li>If the first argument is negative infinity and the second argument is positive
    * infinity, then the result is the double value closest to <code>135.0</code>.
    * <li>If the first argument is positive infinity and the second argument is negative
    * infinity, then the result is the double value closest to <code>-45.0</code>.
    * <li>If both arguments are negative infinity, then the result is the double value
    * closest to <code>-135.0</code>.
    * </ul>
    * <p>
    * The computed result must be within 2 ulp of the exact result. Results must be
    * semi-monotonic.
    * <p>
    * Note: "compass" is the coordinate system used in Robocode and is very similar to
    * polar but with some differences:
    * <ul>
    ...
    * @return the <i>theta</i> component of the point (r, <i>theta</i>) in compass
    ...
    * <li> If one of the values of the array is <code>NaN</code>, then the result is
    * <code>NaN</code>.
    * </ul>
    * 
   ...
   /**
    * Returns the weighed average of the array to <code>double</code> precision. Special
    * cases:
    * <ul>
   ...
   /**
    * Returns the square of the <code>double</code>. That is, the double multiplied be
    * it's self once.
    * 
   ...
   /**
    * Returns the cube of a <code>double</code>. That is, the double multiplied be
    * itself three times.
    * 
   ...
   public static final double getAngle(double deltaX, double deltaY) {
   ...
   /**
    * Returns the angle from the first robot to the second. That is, the angle from
    * <code>r1</code> to <code>r2</code>.
    * 
    * @param r1 - the first robot
    * @param r2 - the second robot
    * @return the angle from the first robot to the second
    */
   public static final double getAngle(RobotData r1, RobotData r2) {
      return getAngle(r1.getX(), r1.getY(), r2.getX(), r2.getY());
   }

   /**
    * Returns the angle from the first point to the second. That is, the angle from
    * <code>p1</code> to <code>p2</code>.
    * 
   ...
   /**
    * Returns the angle offset that the guess factor will introduce. That is, the angle
    * differing from the the robots position with relation to the guess factor and the
   ...
   public static final double getAngleOffset(double x, double y, RobotData robot, double guessFactor, double firePower) {
      double d = getDirection(robot.getHeading(), getAngle(x, y, robot.getX(), robot.getY()), robot.getVelocity());
   ...
   /**
    * Returns the rotation direction. That is, the direction a robot is traveling around a
    * point with 1 being clockwise, and -1 being counter-clockwise.
    * 
   ...
   /**
    * Returns the maximum escape angle. That is, the greatest angle that a robot well
    * achieve before a bullet reaches it; with relation to the robot's position at time of
    * fire and bullet's firing location.
    * 
   ...
   public static final double getDist(double deltaX, double deltaY) {
      return Math.sqrt(getDistSq(deltaX, deltaY));
   }
   ...
   public static final double getDist(Point2D p1, Point2D p2) {
   ...
   public static final double getGuessFactor(VirtualWave wave, RobotData start, RobotData finish) {
      double angleToStart = getAngle(wave.getStartX(), wave.getStartY(), start.getX(), start.getY());
      double direction = getDirection(start.getHeading(), start.getVelocity(), angleToStart);

      double angleToFinish = getAngle(wave.getStartX(), wave.getStartY(), finish.getX(), finish.getY());
   ...
   public static final Point2D getPoint(Point2D origin, double angle, double distance) {
      return new Point2D.Double(getX(origin.getX(), distance, angle), getY(origin.getY(), distance, angle));
   }
   ...
   public static final double getDeltaX(double velocity, double angle) {
   ...
   public static final double getDeltaY(double velocity, double angle) {
      return velocity * cos(angle);
   }

   /**
    * Returns <code>value</code> limited to the <code>min</code> and <code>max</code>.
    * That is, if <code>value</code> is greater then <code>max</code>,
    * <code>max</code> will be returned. If <code>value</code> is less then
    * <code>min</code>, <code>min</code> will be returned. Otherwise
   ...
   /**
    * Returns the rolling average. That is, the weighted average of the current average
    * and the new reading.
    * 
    ...
    * @see <a href="http://robowiki.net/cgi-bin/robowiki?RollingAverage">Rolling Average</a>,
    *      <a href="http://robowiki.net/cgi-bin/robowiki?Paul_Evans">Paul Evans</a>
    * @author Paul Evans
    */
   ...
   public static final <E extends Segmentable, F extends Data, G extends Data> DataWave<E, F, G> findWaveMatch(List<DataWave<E, F, G>> waves,
      ...
      for (DataWave<E, F, G> w; i < waves.size() && wave == null; i++) {
         w = waves.get(i);
         double bulletDistSq = Utils.getDistSq(w.getStartX(), w.getStartY(), bullet.getX(), bullet.getY());
