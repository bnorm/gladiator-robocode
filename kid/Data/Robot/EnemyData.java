...
public class EnemyData extends TeammateData implements java.io.Serializable {
    ...
    public void addPatternMatching(AdvancedRobot MyRobot) {
        ...
        kid.Segmentation.Segmentars.Segmentar[] PatternSegmentarArray = {
                new DistSegmentar(MyRobot), new LateralVelocitySegmentar()
        };
    ...
    public void drawGuessFactorData(RobocodeGraphicsDrawer g, int robotNum) {
        ...
        for (int s = 0; s < FlatBin.numBins * multi; s++) {
            double x = sx + s;
            int index1 = (int) (s / multi), index2 = (int) (s / multi) + 1;
            double height = Utils.weightedAvg(FlatBin.get(index1), index2 - (s / multi), FlatBin.get(index2),
                    (s / multi) - index1);
            double y = Math.max(height, 0) * 2 + sy;
            // if (s / multi >= FlatBin.midBin && s / multi <= FlatBin.midBin +
            // 1) {
            // g.setColor(Colors.YELLOW);
            // } else if (s > FlatBin.midBin) {
    ...
    private boolean fireBullet() {
        if (DELTA_ENERGY >= -3.0 && DELTA_ENERGY < 0.0) {
