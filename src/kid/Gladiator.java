...
public class Gladiator extends AdvancedRobot {
    ...
    public void run() {
        ...
        if (DO_TARGETING) {
            kid.Segmentation.Segmentars.Segmentar[] TargetingSegmentarArray = {
                    new DistSegmentar(this), new WallSegmentar(this, 10, 0), new LateralVelocitySegmentar(),
            ...
            kid.Targeting.Targeting[] TargetingArray = {
                    ...
                    new Linear_wAvgVelocity(this), new Random(this), new GuessFactor(this), new TidalWave(this),
                    new PatternMatchingPolar(this), new PatternMatchingLatVel(this)
            };
        ...
        if (DO_MOVEMENT) {
            kid.Segmentation.Segmentars.Segmentar[] MovementSegmentarArray = {
                    new DistSegmentar(this), new WallSegmentar(this, 10, 0), new LateralVelocitySegmentar(),
                    new AdvancingVelocitySegmentar(), new VelocityChangeTimeSegmentar(), new BearingSegmentar()
            };
    ...
    private double getFirePower(EnemyData Enemy) {
        ...
        if (getOthers() > 1) {
            firepower *= Math.min((getTime() + 1) / 50, 1);
            firepower *= Math.max(getOthers() / 5, 1);
        } else {
            try {
                firepower *= Data.getColsestEnemy().getTopVirtualGun().getHitRate() * 10;
            } catch (Exception e) {
            }
            if (getEnergy() < 15)
