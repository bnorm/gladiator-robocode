...
public class MovementProfile implements java.io.Serializable {
    ...
    private static Node[] Segments = null;
    ...
    public void startDataProsseser(AdvancedRobot MyRobot, DataManager Data, Segmentar[] Sementars) {
        ...
        if (Segments == null && Sementars != null) {
            Segments = new Node[Sementars.length];
            for (int i = 0; i < Segments.length; i++) {
                Segments[i] = new Node(Sementars[i], new GFBin());
            }
    ...
    public void drawSectors(RobocodeGraphicsDrawer g, EnemyData enemy) {
        ...
        for (int s = 0; s < Sectors.length; s++) {
            ...
            g.fillOval((int) (distspred + distspred * s), (int) y, (int) Math.max(
                    ((double) Sectors[s] / Sectors[bestindex]) * 10, 2), (int) Math.max(
                    ((double) Sectors[s] / Sectors[bestindex]) * 10, 2));
            g.setColor(Colors.WHITE);
            g.drawString(String.valueOf(Sectors[s]), (int) (distspred + distspred * s) - 2, (int) (y - 10));
    ...
    public void drawBulletWaves(RobocodeGraphicsDrawer g) {
        ...
        for (int w = 0; w < Waves.length; w++) {
            ...
            for (int s = 0; s < sectors.length; s++) {
                ...
                g.fillOval((int) Utils.getX(wsx, dist, angle + angleOffset), (int) Utils.getY(wsy, dist, angle
