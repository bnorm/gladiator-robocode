...
public class RobotMovement {
    ...
    public final MyRobotData AjustHeadingAndPointToSmoothWalls(MyRobotData r, double targetVelocity, double targetHeading,
            long time) {
        double radius = i.getRobotTurnRadius(MyRobotsInfo.MAX_VELOCITY);
        double bfh = i.getBattleFieldHeight(), bfw = i.getBattleFieldWidth();
        for (int t = 0; t < time; t++) {
            double mX = Utils.getX(r.getX(), i.getFutureVelocity(r.getVelocity(), targetVelocity),
                                   i.getFutureRobotFrontHeading(r.getHeading(), r.getVelocity(), targetHeading));
            double mY = Utils.getY(r.getY(), i.getFutureVelocity(r.getVelocity(), targetVelocity),
                                   i.getFutureRobotFrontHeading(r.getHeading(), r.getVelocity(), targetHeading));
            ...
            double robotturn = Utils.sign(Utils.relative(targetHeading - r.getHeading())) *
                               i.getRobotTurnRate(r.getVelocity());
            double getfrontheading = i.getFutureRobotFrontHeading(r.getHeading(), r.getVelocity(), robotturn);
            double getfbackheading = Utils.oppositeRelative(getfrontheading);

            double turnAngle = 0;
            if (i.getRobotMovingSign() > -1) {
                if (isAngleBetween(getfrontheading, LAngle, RAngle) ||
                    (!isAngleBetween(getfrontheading, LAngle, RAngle) && twoWalls)) {
                    if (Math.abs(frb) <= Math.abs(flb)) {
                        turnAngle = frb;
                    } else {
                        turnAngle = flb;
                ...
                if (isAngleBetween(getfbackheading, LAngle, RAngle) ||
                    (!isAngleBetween(getfbackheading, LAngle, RAngle) && twoWalls)) {
                    if (Math.abs(brb) <= Math.abs(blb)) {
                        turnAngle = brb;
                    } else {
                        turnAngle = blb;
            ...
            double v = i.getFutureVelocity(r.getVelocity(), targetVelocity);
            double h = i.getFutureRobotFrontHeading(r.getHeading(), v, turnAngle);
            double x = Utils.getX(r.getX(), v, h);
            double y = Utils.getY(r.getY(), v, h);

            r.updateItem(x, y, 0, 0, h, v, 0);
        }
