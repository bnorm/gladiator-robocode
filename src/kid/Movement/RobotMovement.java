...
public class RobotMovement {
    ...
    public void ajustVectorForWall(RobotVector v) {
        ...
        double frb = Utils.relative(RAngle - h); // FRONT RIGHT BEARING
        double flb = Utils.relative(LAngle - h); // FRONT LEFT BEARING
        double brb = Utils.oppositeRelative(RAngle - h); // BACK RIGHT BEARING
        double blb = Utils.oppositeRelative(LAngle - h); // BACK LEFT BEARING
        ...
        if (v.getVelocity() > -1) {
            ...
            if (isAngleBetween(Utils.oppositeRelative(h), LAngle, RAngle) ||
                (!isAngleBetween(Utils.oppositeRelative(h), LAngle, RAngle) && twoWalls &&
                 Utils.absolute(RAngle + (360 - LAngle)) > 180)) {
                if (Math.abs(brb) <= Math.abs(blb)) {
                    ajustangle = brb;
                } else {
