            ...
            if (EnemyData != null) {
                ...
                if (HighRiskWaves == null || f.getClosestWave() == null || HighRiskWaves.length == 0) {
                    ...
                    for (int w = 0, nwaves = HighRiskWaves.length; w < nwaves && w < MAX_WAVES; w++) {
                        ...
                        if (false) { // HeadOn avodence.
                            double waverisk = Math.abs(wearydist
                                    / (wave.distToImpact(MyRobot) - MyRobotsInfo.LENGTH_CONER)) * 2;
                            double maxrisk = 0;
                        ...
                        for (;; ft++) {
                            forwards.moveVector(Movement.getAngleForPerpenToRobotwDC(forwards, targetVelFor, EnemyData,
                                    SafeDist(SafeDist, EnemyData)), targetVelFor);
                            ...
                            double angletoside = Utils.atan(MyRobotsInfo.LENGTH_CONER
                        ...
                        for (;; bt++) {
                            ...
                            double angletoside = Utils.asin(MyRobotsInfo.LENGTH_CONER
                                    / Utils.getDist(wave.getStartX(), wave.getStartY(), x, y)) / 2;
                            int indextoside = (Utils.getIndex(angletoside / wave.maxEscapeAngle(), numsec) - midsec);
