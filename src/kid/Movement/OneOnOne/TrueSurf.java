            ...
            if (bulletWaves == null || MovementProfile.getClosestWave() == null || bulletWaves.length == 0) {
                ...
                for (int w = 0, nwaves = bulletWaves.length; w < nwaves && w < MAX_WAVES; w++) {
                    ...
                    for (;; ft++) {
                        ...
                        for (int r = w; r < bulletWaves.length && r < MAX_WAVES; r++) {
                            ...
                            for (int i = 0; i < indextoside * 2 + 1; i++) {
                                wriskf[r] +=
                                // Math.abs(dist - WantDist) *
                                sectors[Math.min(numsec - 1, Math.max(0, index + (i - indextoside)))];
                            }
                        }
                    }
                    ...
                    if (ft != 0) {
                        wriskf[w] /= ft;
                        // wriskf[w] *=
                        // Math.abs(Utils.getDist(HighRiskWaves[w].getStartX(),
                        // HighRiskWaves[w].getStartY(),
                ...
                for (int w = 0; w < wriskf.length && w < bulletWaves.length; w++) {
                    double dist = bulletWaves[w].distSqToImpact(MyRobot);
                    if (dist <= 0.0)
                        continue;
                    // d.println("Wave: " + w);
                ...
                Movement.setTurnPerpenToXYwBFwDCwRM(MovementProfile.getClosestWave().getStartX(), MovementProfile.getClosestWave().getStartY(),
                        WantDist);
            }
