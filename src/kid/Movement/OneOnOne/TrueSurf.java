...
public class TrueSurf extends WaveSurfing {
        ...
        if (MyRobot instanceof AdvancedRobot) {
            if (EnemyData != null) {
                ...
                if (HighRiskWaves == null || f.getMaxRiskWave() == null || HighRiskWaves.length == 0) {
                    ...
                    long maxtime = 1;
                    for (int w = 0; w < HighRiskWaves.length && w < 2; w++)
                        maxtime = (long) (Math.max(maxtime, HighRiskWaves[w].timeToImpact(MyRobot)) * 1.5);
                    ...
                    for (int t = 1; t < maxtime; t++) {
                        double targetVel = Utils.sign(startvelocity) * MyRobotsInfo.MAX_VELOCITY;
                        RobotVector v = forwards[t - 1].getNextVector(Movement.getAngleForPerpenToRobotwDC(
                                forwards[t - 1], targetVel, EnemyData, SafeDist(SafeDist, EnemyData)), targetVel);
                        Movement.ajustVectorForWall(v);
                        forwards[t] = v;
                    }
                    velocity = i.getVelocity();
                    for (int t = 1; t < maxtime; t++) {
                        double targetVel = Utils.sign(startvelocity) * -1 * MyRobotsInfo.MAX_VELOCITY;
                        RobotVector v = backwards[t - 1].getNextVector(Movement.getAngleForPerpenToRobotwDC(
                                backwards[t - 1], targetVel, EnemyData, SafeDist(SafeDist, EnemyData)), targetVel);
                        Movement.ajustVectorForWall(v);
                        backwards[t] = v;
                    }
                    ...
                    for (int w = 0; w < HighRiskWaves.length && w < 2; w++) {
                        ...
                        for (; bt < backwards.length; bt++) {
                            ...
                            for (int i = 0; i < indextoside * 2 + 1 && index + (i - indextoside) < sectors.length
                                    && index + (i - indextoside) > -1; i++) {
                                wriskb += sectors[index + (i - indextoside)];
                            }
                            if (wave.testHit(new Rectangle2D.Double(x, y, MyRobotsInfo.WIDTH, MyRobotsInfo.HEIGHT),
                                    time + bt)) {
                                break;
                        ...
                        wriskf /= ft;
                        wriskb /= bt;
                        wriskf *= 1 / Math.abs(wave.distSqToImpact(MyRobot));
                        wriskb *= 1 / Math.abs(wave.distSqToImpact(MyRobot));
                        riskf += wriskf;
                        riskb += wriskb;
                    }
                    if (riskf > riskb) {
    ...
    public void drawMovement(RobocodeGraphicsDrawer g, EnemyData EnemyData) {
        ...
        for (int t = 1; t < maxtime; t++) {
            double targetVel = Utils.sign(startvelocity) * MyRobotsInfo.MAX_VELOCITY;
            RobotVector v = forwards[t - 1].getNextVector(Movement.getAngleForPerpenToRobotwDC(forwards[t - 1],
                    targetVel, EnemyData, SafeDist(SafeDist, EnemyData)), targetVel);
            Movement.ajustVectorForWall(v);
            forwards[t] = v;
        }
        velocity = i.getVelocity();
        for (int t = 1; t < maxtime; t++) {
            double targetVel = Utils.sign(startvelocity) * -1 * MyRobotsInfo.MAX_VELOCITY;
            RobotVector v = backwards[t - 1].getNextVector(Movement.getAngleForPerpenToRobotwDC(backwards[t - 1],
                    targetVel, EnemyData, SafeDist(SafeDist, EnemyData)), targetVel);
            Movement.ajustVectorForWall(v);
            backwards[t] = v;
        }
        ...
        for (int ft = 0; ft < forwards.length; ft++) {
            g.fillOvalCenter((int) forwards[ft].getX(), (int) forwards[ft].getY(), 2, 2);
        }
        g.setColor(Colors.RED);
        for (int bt = 0; bt < backwards.length; bt++) {
            g.fillOvalCenter((int) backwards[bt].getX(), (int) backwards[bt].getY(), 2, 2);
        }
