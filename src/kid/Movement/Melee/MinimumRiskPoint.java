...
public class MinimumRiskPoint extends MeleeMovement {
    ...
    private double getPointRisk(Point2D p, EnemyData[] EnemyData) {
        ...
        double minx = 0;
        if (Math.abs(MyRobot.getBattleFieldWidth() - p.getX()) < p.getX())
            minx = MyRobot.getBattleFieldWidth();
        else
            minx = 0.0;
        double miny = 0;
        if (Math.abs(MyRobot.getBattleFieldHeight() - p.getY()) < p.getY())
            miny = MyRobot.getBattleFieldHeight();
        else
            miny = 0.0;
        risk += CORNER_RISK / p.distanceSq(minx, miny);

        // risk += CORNER_RISK / p.distanceSq(0, i.getBattleFieldHeight());
        // risk += CORNER_RISK / p.distanceSq(0, 0);
        // risk += CORNER_RISK / p.distanceSq(i.getBattleFieldWidth(), 0);

        // risk += 2 / p.distanceSq(0, i.getBattleFieldHeight() / 2);
        // risk += 2 / p.distanceSq(i.getBattleFieldWidth() / 2,
        // i.getBattleFieldHeight());
        // risk += 2 / p.distanceSq(i.getBattleFieldWidth() / 2, 0);
        // risk += 2 / p.distanceSq(i.getBattleFieldWidth(),
