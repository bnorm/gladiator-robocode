...
public class GuessFactor extends StatisticalTargeting {
    ...
    public double getTargetingAngle(EnemyData EnemyRobot, double FirePower) {
        ...
        angleOffset = Utils.rollingAvg(EnemyRobot.getAvgAngleOffSet(FirePower), angleOffset, EnemyData.AvgLength / 2, FirePower);
    ...
    public void drawTargeting(RobocodeGraphicsDrawer g, EnemyData EnemyRobot, double FirePower) {
        ...
        double guessfactor = Utils.getGuessFactor(bestindex, sectors.length);
        double angleOffset = Utils.getAngleOffset(MyRobot, EnemyRobot, guessfactor, FirePower);
        double rollingangleOffset = Utils.rollingAvg(EnemyRobot.getAvgAngleOffSet(FirePower), angleOffset, EnemyData.AvgLength, FirePower);

        g.setColor(Colors.GREEN);
        g.drawLine(i.getX(), i.getY(), Utils.getX(i.getX(), dist, angle + angleOffset), Utils.getY(i.getY(), dist, angle + angleOffset));
        g.setColor(Colors.RED);
        g.drawLine(i.getX(), i.getY(), Utils.getX(i.getX(), dist, angle + rollingangleOffset), Utils.getY(i.getY(), dist, angle + rollingangleOffset));
        g.setColor(Colors.BLUE);
        g.drawLine(i.getX(), i.getY(), Utils.getX(i.getX(), dist, angle + EnemyRobot.getAvgAngleOffSet(FirePower)), Utils.getY(i.getY(), dist, angle + EnemyRobot.getAvgAngleOffSet(FirePower)));
